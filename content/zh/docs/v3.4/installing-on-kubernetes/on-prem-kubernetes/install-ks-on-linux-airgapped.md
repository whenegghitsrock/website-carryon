...
1. 执行以下命令下载这两个文件，并将它们传输至您充当任务机的机器，用于安装。
   ...
   ```bash
   curl -L -O https://github.com/whenegghitsrock/ks-installer-carryon/releases/download/v3.4.1/cluster-configuration.yaml
...
3. 编辑完成后保存 `cluster-configuration.yaml`。使用以下命令将 `ks-installer` 替换为您**自己仓库的地址**。
   ...
   ```bash
   sed -i "s#^\s*image: kubesphereon.*/ks-installer:.*#        image: dockerhub.kubekey.local/kubesphere/ks-installer:v3.4.0#" kubesphere-installer.yaml
   ```
