...
In this tutorial, the example pipeline will deploy the [sample](https://github.com/whenegghitsrock/devops-maven-sample/tree/sonarqube) app to a project. Hence, you must create the project (for example, `kubesphere-sample-dev`) in advance. The Deployment and Service of the app will be created automatically in the project once the pipeline runs successfully.
...
3. Click **Add Step**. Select **git** from the list as the example code is pulled from GitHub. In the displayed dialog box, fill in the required field. Click **OK** to finish.
   ...
   - **URL**. Enter the GitHub repository address `https://github.com/whenegghitsrock/devops-maven-sample.git`. Note that this is an example and you need to use your own repository address.
...
4. Click **Add Nesting Steps** again and select **shell**. Enter the following command in the command line to build a Docker image based on the [Dockerfile](https://github.com/whenegghitsrock/devops-maven-sample/blob/sonarqube/Dockerfile-online). Click **OK** to confirm.
